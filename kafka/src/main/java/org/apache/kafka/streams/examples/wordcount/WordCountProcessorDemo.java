...
public final class WordCountProcessorDemo {
    ...
    public static void main(final String[] args) throws IOException {
        final KafkaStreams streams = new KafkaStreams(getTopology(), getProperties(args));
        startStreams(streams);
        System.exit(0);
    }

    private static void startStreams(KafkaStreams streams) {
        final CountDownLatch latch = addHook(streams);
        try {
            streams.start();
            latch.await();
    ...
    private static CountDownLatch addHook(KafkaStreams streams) {
